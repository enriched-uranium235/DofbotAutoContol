...
```
git clone https://github.com/enriched-uranium235/DofbotAutoContol.git
```
...
```
  ...
  - dofbot_app_connect_button_coodinateはツール起動後自動で立ち上がるDofbot制御アプリのIPアドレスとポート番号の設定画面を開くために必要です。
...
- config.tomlの設定が終わったら以下のコマンドでサーバーを起動します。
```
